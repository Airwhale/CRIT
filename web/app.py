...
async def get_status(session_id: str | None = Cookie(default=None)):
    ...
    return {
        ...
        "steam_api_key":     steam_creds.get("api_key") if steam_creds else None,
...
async def clear_library_cache(session_id: str | None = Cookie(default=None)):
    """Clear persisted and in-session library cache data."""
    _, session = _get_session(session_id)
    session.pop("library", None)

    removed = False
    try:
        if _LIBRARY_CACHE_FILE.exists():
            _LIBRARY_CACHE_FILE.unlink()
            removed = True
    except Exception as e:
        raise HTTPException(500, f"Failed to clear library cache: {e}")
